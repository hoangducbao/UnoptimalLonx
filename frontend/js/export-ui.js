...
// module importing that tab's state.js), and an `onDone(reason, filename?)`
// callback for "the user is finished with this UI" (cancel or a completed
// export) instead of assuming a dialog overlay it can remove.
//
...
import { openTrakePlaybackDialog } from "./dialogs.js";
...
// Item 5's "Change" typing boxes: `L21_V001`-shaped video id, plain-integer
// frame id (the leading zeros in the "001" placeholder are just display
// convention -- parseInt handles them fine either way). Return null on a
// malformed box so the caller can show one "correct format" error rather
// than letting a bad value reach the backend as a confusing 404/422.
...
export function buildExportUI(container, trigger, { getCandidates, onDone }) {
    ...
    box.innerHTML = `
        ...
        <div id="exp-flat-body">
          <div class="export-change-row" id="exp-change-row">
            <input type="text" id="exp-change-video" placeholder="Video ID, e.g. L21_V001">
            <input type="text" id="exp-change-frame" placeholder="Frame ID, e.g. 001">
    ...
    async function renderPreview() {
        ...
        // TRAKE has no single "current export frame" (it's per-event native
        // frame_idx, a different model entirely) -- only offer replace for
        // KIS/VQA's confirmed mode.
        const replaceable = s.confirmed && s.queryType !== "TRAKE";
    ...
    function renderTrakeContent() {
        ...
        if (s.trigger.kind !== "trake") {
            content.innerHTML = `<div class="status-banner info">Open this from a TRAKE candidate's ★ button.</div>`;
            return;
        }
    ...
    // Item 5 -- typed "Video ID" / "Frame ID" boxes + Change button: same
    // destination as item 4's preview-pick (applyChangedFrame), but reaches
    // an arbitrary frame not necessarily in either preview list. Verifies
    // the frame actually exists (via /api/export/frame) before applying, so
    // a typo lands as one clear error rather than a broken export.
    el("#exp-change-btn").onclick = async () => {
        const errEl = el("#exp-error");
        errEl.style.display = "none";

        const videoId = parseVideoIdInput(el("#exp-change-video").value);
        const n = parseFrameIdInput(el("#exp-change-frame").value);
        if (!videoId || !n) {
            errEl.textContent = "Enter a valid video id (e.g. L21_V001) and frame id (e.g. 001).";
        ...
        btn.disabled = true;
        try {
            await getExportFrame(videoId, n); // throws if that frame doesn't exist for that video
            applyChangedFrame({ video_id: videoId, n });
    ...
    el("#exp-export").onclick = async () => {
        ...
        if (s.queryType === "TRAKE") {
            if (s.trigger.kind !== "trake") {
                errEl.textContent = "No TRAKE candidate -- open this from a TRAKE result card.";
                errEl.style.display = "block";
                return;
            }
            const candidates = getCandidates();
            ...
            if (s.confirmed && !s.answerFrame) {
                errEl.textContent = "No confirmed frame -- open this from a result card.";
                errEl.style.display = "block";
                return;
            }
